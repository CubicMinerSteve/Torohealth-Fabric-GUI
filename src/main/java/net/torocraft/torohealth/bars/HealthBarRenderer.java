...
public class HealthBarRenderer {
	...
	public static boolean checkEntity(LivingEntity entity) {
	...
	public static void renderInHud(GuiGraphics graphics, LivingEntity entity, double xOffset, double yOffset, float width) {
	...
	public static void renderInWorld(PoseStack poseStack, LivingEntity entity, double cameraX, double cameraY, double cameraZ) {
		// Get entity relation to players.
		Relation relation = EntityUtils.determineRelation(entity);
		...
		drawBarInWorld(positionMatrix, 0, 0, FULL_SIZE, 1, DARK_GRAY, zOffset++);
		drawBarInWorld(positionMatrix, 0, 0, FULL_SIZE, percent2, color2, zOffset++);
		drawBarInWorld(positionMatrix, 0, 0, FULL_SIZE, percent, color, zOffset);

		poseStack.popPose();
	}

	private static void drawBarInHud(GuiGraphics graphics, double xOffset, double yOffset, float width, float percent, int color) {
	...
	private static void drawBarInWorld(Matrix4f matrix4f, double x, double y, float width, float percent, int color, int zOffset) {
		...
		VertexConsumer vertexConsumer = Minecraft.getInstance().renderBuffers().bufferSource().getBuffer(layer);
		vertexConsumer.addVertex(matrix4f, (float) (-half + x), (float) y, zOffset * zOffsetAmount).setUv(u * c, v * c).setColor(hex);
		vertexConsumer.addVertex(matrix4f, (float) (-half + x), (float) (regionH + y), zOffset * zOffsetAmount).setUv(u * c, (v + vh) * c).setColor(hex);
		vertexConsumer.addVertex(matrix4f, (float) (-half + regionW + x), (float) (regionH + y), zOffset * zOffsetAmount).setUv((u + uw) * c, (v + vh) * c).setColor(hex);
		vertexConsumer.addVertex(matrix4f, (float) (-half + regionW + x), (float) y, zOffset * zOffsetAmount).setUv((u + uw) * c, v * c).setColor(hex);
	}

	public static void drawDamageNumber(GuiGraphics graphics, int dmg, double xOffset, double yOffset, float width) {
		...
		graphics.drawString(minecraft.font, s, (int) (width + xOffset) - 2 - sw, (int) yOffset + 10, color);
	}
