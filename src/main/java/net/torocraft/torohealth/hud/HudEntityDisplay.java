...
public class HudEntityDisplay {
	...
	public void draw(GuiGraphics graphics, float xPos, float yPos, float hudScale) {
		if (entity != null) {
			try {
	...
	public static void drawEntity(GuiGraphics graphics, LivingEntity entity, float xPos, float yPos, float hudScale, float entitySize, Vector3f entityBlockOffset) {
		...
		// Substract 5 from x to conform rotataion. Area width is 36 without scaling.
		int left = Math.round(xPos);
		int right = Math.round(xPos + 36 * hudScale);

		// Keep intact for y. Area and height is 36 without scaling.
		...
		graphics.submitEntityRenderState(renderState, hudScale * entitySize, entityBlockOffset, zRotation, xRotation, left, top, right, bottom);
