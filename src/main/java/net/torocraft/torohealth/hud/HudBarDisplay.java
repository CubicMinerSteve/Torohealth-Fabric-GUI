...
public class HudBarDisplay {
	...
	public void draw(GuiGraphicsExtractor graphics, LivingEntity entity) {
		...
		graphics.text(font, name, xOffset, 4, 0xFFE0E0E0);
		xOffset += font.width(name) + 5;

		renderHeartIcon(graphics, xOffset, 4);
		xOffset += 10;

		graphics.text(font, healthText, xOffset, 4, 0xFFE0E0E0);
		...
		if (armor > 0) {
			...
			xOffset += 10;
			graphics.text(font, entity.getArmorValue() + "", xOffset, 4, 0xFFE0E0E0);
		}
	}
	...
	private void renderArmorIcon(GuiGraphicsExtractor graphics, int xOffset, int yOffset) {
		graphics.blit(RenderUtils.TOROHEALTH_HUD_PIPELINE, ARMOR_FULL, xOffset, yOffset, 0, 0, 9, 9, 9, 9);
	}

	private void renderHeartIcon(GuiGraphicsExtractor graphics, int xOffset, int yOffset) {
